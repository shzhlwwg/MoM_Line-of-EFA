...
function [Fcoulombian,x,y,U]=GUIFunc_MatrixChargeCoulF_PMOML(a,d,s,dx,phi,N,M)
...
%% plot potential
[x,y] = meshgrid(-2*p:0.1*p:2*p,-1*d:0.1*d:1*d);
[sx,sy]=size(x);
...
for i =1:sx*sy
    for j = 1:2*3*N
       u(j) =  Func_PMOML(Xp(j,i),Yp(j,i),R,M,p)*q0(j);
    end
    U(i) = sum(u);
